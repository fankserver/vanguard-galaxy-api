...
| **[StationCommerce](StationCommerce/)** | **Your own trade goods and the people who deal in them** — a manufactured item with the maker's own tooltip tip, the recipe that makes it, and a bar contact with a mission, placed at a real station. | `Items`, `RecipeRegistration`, `Bars`, `Story`, `Game`, `Tooltips` |
| **[UiSurfaces](UiSurfaces/)** | **UI, settings and saved progress** — a hosted window with typed global preferences, a per-save counter, a Forge inspector and always-visible annotations on ship-module stat lists and the Engineering mastery badge. Requires the matching development API, not public 0.2.8. | `GameplayUi`, `Hud`, `Settings`, `SaveData`, `ForgeUi`, `Recipes`, `RecipeQuotes`, `Tooltips` |
| **[EquipmentTargeting](EquipmentTargeting/)** | **Your own targeting decision without patches** — an always-active extra beam for the game's tractor autopilot; abstain-by-null, native safety retained. Requires the matching development API, not public 0.2.8. | `Equipment` |
