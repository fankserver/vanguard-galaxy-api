...
| **[UiSurfaces](UiSurfaces/)** | **UI, settings and saved progress** — a hosted window with typed global preferences, a per-save counter, a Forge inspector and opt-in annotations on ship-module stat lists and the Engineering mastery badge. Requires the matching development API, not public 0.2.8. | `GameplayUi`, `Hud`, `Settings`, `SaveData`, `ForgeUi`, `Recipes`, `RecipeQuotes`, `Tooltips` |
| **[EquipmentTargeting](EquipmentTargeting/)** | **Your own targeting decision without patches** — a settings-gated extra beam for the game's tractor autopilot; abstain-by-null, native safety retained. Requires the matching development API, not public 0.2.8. | `Equipment` |
