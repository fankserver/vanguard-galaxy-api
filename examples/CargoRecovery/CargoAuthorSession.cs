...
public sealed class CargoAuthorSession : IDisposable
{
        ...
        Action<string> log, Action<string>? warn = null)
    {
        ...
        void Initialize(bool allowContentAttempt = true)
        {
            ...
            if (_author == null)
            {
                ...
                try { _author = new CargoEncounter(dungeons, Id, reward); _log("Cargo content registered; shipment reward = " + reward + "."); }
            ...
            try
            {
                ...
                _boardingHandler = fact =>
                {
                    if (OwnedTarget != null && fact.Target.Handle.Equals(OwnedTarget)
                        ...
                        && !dungeons.GetOperations().Any(operation => operation.Target.Equals(OwnedTarget)))
                    {
    ...
    {
        if (_dungeons == null || OwnedTarget != null) return;
        var candidates = _dungeons.GetTargets();
        if (candidates.Count != 1) return; // Ambiguous: skip the optional controls rather than guess.
        var target = candidates[0].Handle;
        OwnedTarget = target;

        if (!_dungeons.Capabilities.ContextualActions)
        { _log("Layout attached; the contextual-action renderer is unavailable."); return; }

        _panel = new CargoEncounterPanel(Id, target, _dungeons,
