...
public sealed partial class Plugin
{
    ...
    private void InitializeWorldProtection()
    {
        ...
        try
        {
            ...
            // Constructing the equipment runtime is isolated from the world-protection path: a bind or
            // shape failure only degrades the optional relative features (forced rarity, damage
            // overclock, dynamic level scaling fall back to vanilla) — it must never unpin world
            // protection or any other capability installed in the enclosing all-or-nothing block.
            VGModAPI.Core.Integration.EncounterEquipmentRuntime? encounterEquipment = null;
            try { encounterEquipment = new VGModAPI.Core.Integration.EncounterEquipmentRuntime(assembly, fault => Logger.LogWarning("Encounter equipment override fault: " + fault)); }
            catch (Exception equipmentError) { Logger.LogWarning("Encounter equipment override unavailable (degree only): " + equipmentError.Message); }
            ...
            _worldContent = new WorldContentService(_hub, definitions, new WorldAuthoringGate(definitions, creation, _worldPersistence.CanMutate), admission, () => { try { _story?.RefreshWorldDependencies(); } finally { _worldLifetimeHost?.MaintainActors(); } }, _ambientTraffic ??= new AmbientTrafficService(_hub), _unitProtection ??= new UnitProtectionService(_hub), _droneBays ??= new DroneBayService(_hub), _authoredDefinitions, authored, _siteDefinitions, _siteCoordinator, _shipDefinitions, _shipCoordinator,
                _siteCoordinator == null ? null : new WorldNativeEncounters(_adapter, assembly, equipment: encounterEquipment, report: fault => Logger.LogError(fault)), _wormholeDefinitions, _wormholeCoordinator);
