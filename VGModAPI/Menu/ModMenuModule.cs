...
internal sealed class ModMenuModule : IDisposable
{
    ...
    internal ModMenuModule(Assembly assembly, ModInformationCatalog catalog, Func<string> diagnostics, Action<Exception> fault)
    {
        ...
        _lifetime = new ModMenuLifetime((menu, viewport, canvas) => ModMenuView.Create(
            ...
            new ModInformationPresenter(catalog), diagnostics, fault));
    }
