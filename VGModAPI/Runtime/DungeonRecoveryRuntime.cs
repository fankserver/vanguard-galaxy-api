...
internal sealed class DungeonRecoveryRuntime : IDisposable
{
    ...
    internal Func<object, Guid?>? ContentDungeon { get; set; }
    ...
    internal bool CaptureOperation(object operation, bool fresh = false)
    {
        ...
        if (!id.HasValue)
        {
            ...
            else
            {
                ...
                id = Operations.Created(operation, ContentDungeon?.Invoke(location), token.Length == 0 ? mission : token + (mission.Length == 0 ? "" : "|" + mission));
            }
        }
        ...
        if (!State.TrackOperation(new(previous.Id, previous.LocationId, previous.ContentDungeon, previous.AttackerShipId, previous.DungeonType,
