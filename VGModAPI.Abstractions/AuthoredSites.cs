...
/// </summary>
public sealed class AuthoredSiteDefinition
{
    public string LocalId { get; }
    public int Revision { get; }
    public string Name { get; }
    public AuthoredSiteKind Kind { get; }
    ...
    public AuthoredSiteHazard? Hazard { get; }
    ...
    private AuthoredSiteDefinition(string localId, int revision, string name, AuthoredSiteKind kind, int level,
        string? factionId, string? wreckShipId, bool withStation, AuthoredSiteHazard? hazard, bool scatterAsteroids, int asteroidCount)
    {
        ...
        if (hazard.HasValue && !Enum.IsDefined(typeof(AuthoredSiteHazard), hazard.Value)) throw new ArgumentOutOfRangeException(nameof(hazard));
    ...
    public static AuthoredSiteDefinition Salvage(string localId, int revision, string name, int level, string wreckShipId,
        string factionId, bool withStation = false, AuthoredSiteHazard? hazard = null, bool scatterAsteroids = false)
    {
        ...
        return new AuthoredSiteDefinition(localId, revision, name, AuthoredSiteKind.SalvageSite, level,
            factionId, wreckShipId, withStation, hazard, scatterAsteroids, 0);
    }

    public static AuthoredSiteDefinition MiningField(string localId, int revision, string name, int level, int asteroidCount)
    {
        if (asteroidCount < 1 || asteroidCount > 64) throw new ArgumentOutOfRangeException(nameof(asteroidCount));
        return new AuthoredSiteDefinition(localId, revision, name, AuthoredSiteKind.MiningField, level,
...
public sealed class AuthoredSiteState
{
    ...
    public AuthoredSiteState(AuthoredSystemReconstructionStatus status, AuthoredSystemFailureReason? reason = null, string? poiId = null)
...
/// </summary>
public interface IAuthoredSite
{
    string OccurrenceKey { get; }
    AuthoredSiteDefinition Definition { get; }
    AuthoredSiteState State { get; }
    string? PoiId { get; }
    AuthoredActionResult LastAction { get; }
    event Action<IAuthoredSite>? Changed;
}

public sealed class AuthoredSiteFailure
{
    public IAuthoredSite Occurrence { get; }
    public AuthoredSystemFailureReason Reason { get; }
    public AuthoredSiteFailure(IAuthoredSite occurrence, AuthoredSystemFailureReason reason)
...
public sealed class AuthoredSitesSettledEvent
{
    public Guid SessionId { get; }
    public IReadOnlyList<IAuthoredSite> Reconstructed { get; }
    public IReadOnlyList<AuthoredSiteFailure> Failures { get; }
    public bool HasFailures => Failures.Count > 0;
    public AuthoredSitesSettledEvent(Guid sessionId, IEnumerable<IAuthoredSite> reconstructed, IEnumerable<AuthoredSiteFailure> failures)
    {
        SessionId = sessionId;
        Reconstructed = new ReadOnlyCollection<IAuthoredSite>(new List<IAuthoredSite>(reconstructed ?? throw new ArgumentNullException(nameof(reconstructed))));
        Failures = new ReadOnlyCollection<AuthoredSiteFailure>(new List<AuthoredSiteFailure>(failures ?? throw new ArgumentNullException(nameof(failures))));
    }
}
