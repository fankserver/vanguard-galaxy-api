...
public sealed class InstalledRecipeBindingTests
{
    ...
    public void RecipeReadsBindToInspectedMemberShapes()
    {
        ...
        foreach (var spec in RecipeCatalogBindings.Members.Concat(RecipeQuoteBindings.Members).Concat(CraftingJobBindings.Members).Concat(CraftingCommandBindings.Members))
        {
        ...
        foreach (var spec in RecipeQuoteBindings.Methods.Concat(CraftingJobBindings.Hooks).Concat(CraftingJobBindings.Persistence).Concat(CraftingCommandBindings.Actions).Concat(CraftingCommandBindings.Serialization))
        {
            var type = assembly.MainModule.GetType(spec.Type);
