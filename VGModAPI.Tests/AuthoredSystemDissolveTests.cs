...
public sealed class AuthoredSystemDissolveTests
{
    ...
    private sealed class Harness : IDisposable
    {
        ...
        internal readonly FakeAuthoredSiteNative SiteNative;
        ...
        internal readonly AuthoredSiteCoordinator SiteCoordinator;
        ...
        internal Harness(bool withCombatGate = false)
        {
            ...
            SiteNative = new FakeAuthoredSiteNative();
            ...
            Coordinator = new AuthoredSystemCoordinator(Hub, Systems, Native, () => true, _ => true, _ => { });
            Sites = new AuthoredSiteRegistry(auth, Hub.CheckThread);
            SiteCoordinator = new AuthoredSiteCoordinator(Hub, Sites, SiteNative, _ => true, _ => { });
        ...
        internal IAuthoredSystem CreatePocket(string key = "k1")
        {
            Assert.Equal(WorldStatus.Succeeded, Provider.RegisterAuthoredSystem(new AuthoredSystemDefinition("pocket", 1, "The Hollow")));
            BeginGameplay();
            return Provider.CreateAuthoredSystem("pocket", key, "anchor")!;
        }
        public void Dispose()
        {
            Provider?.Dispose();
    ...
    public void DissolveRemovesThePocketItsSaveRowAndFreesTheKeyForAFreshOccurrence()
    {
        ...
        Assert.Equal(AuthoredSystemReconstructionStatus.Dissolved, pocket.State.Status);
        Assert.Null(pocket.SystemId); Assert.Null(pocket.EntranceGatePoiId); Assert.Null(pocket.PocketGatePoiId);
        Assert.Equal(AuthoredActionStatus.Rejected, pocket.SetEntranceOpen(true).Status);
        Assert.Equal(AuthoredActionStatus.Rejected, pocket.Dissolve().Status);
        Assert.Equal(AuthoredActionStatus.Rejected, pocket.LastAction.Status);
        // The occurrence is no longer obtainable; the key authors a FRESH pocket with fresh native identity.
        Assert.Null(harness.Provider.GetAuthoredSystem("pocket", "k1"));
        Assert.Empty(harness.Provider.GetAuthoredSystems("pocket"));
        var fresh = harness.Provider.CreateAuthoredSystem("pocket", "k1", "anchor")!;
        Assert.NotSame(pocket, fresh);
        Assert.Equal("sys-2", fresh.SystemId);
        Assert.Equal(AuthoredSystemReconstructionStatus.Reconstructed, fresh.State.Status);
        Assert.Equal(AuthoredSystemReconstructionStatus.Dissolved, pocket.State.Status);
    }
    ...
    public void PlayerInsideThePocketRefusesDissolutionAndRetainsTheOccurrence()
    {
        ...
        Assert.Equal(AuthoredActionStatus.Rejected, refused.Status);
        ...
        Assert.Equal(AuthoredSystemReconstructionStatus.Reconstructed, pocket.State.Status);
    ...
    public void NativeFaultIsReportedAsUnavailableAndRetainsTheRow()
    {
        ...
        Assert.Equal(AuthoredActionStatus.Unavailable, pocket.Dissolve().Status);
        Assert.Single(harness.Coordinator.CaptureRows());
    ...
    {
        using var harness = new Harness();
        Assert.Equal(WorldStatus.Succeeded, harness.Provider.RegisterAuthoredSystem(new AuthoredSystemDefinition("pocket", 1, "The Hollow")));
        harness.BeginGameplay();
        harness.Native.FailCreate = true;
        var failed = harness.Provider.CreateAuthoredSystem("pocket", "k1", "anchor")!;
    ...
    public void ReplacedSessionRefusesDissolveWithGameEnded()
    {
        ...
        harness.BeginGameplay();
        Assert.Equal(AuthoredActionStatus.GameEnded, pocket.Dissolve().Status);
    }

    [Fact]
    public void AuthoredSitesInsideTheDissolvedPocketAreDroppedAndTheirObjectsAreTerminal()
    {
        using var harness = new Harness();
        Assert.Equal(WorldStatus.Succeeded, harness.Provider.RegisterAuthoredSystem(new AuthoredSystemDefinition("pocket", 1, "The Hollow")));
        Assert.Equal(WorldStatus.Succeeded, harness.Provider.RegisterAuthoredSite(
            AuthoredSiteDefinition.MiningField("field", 1, "Singer's Field", 8, 6)));
        harness.BeginGameplay();
        var pocket = harness.Provider.CreateAuthoredSystem("pocket", "k1", "anchor")!;
        var inside = harness.Provider.CreateAuthoredSite("field", "in-pocket", pocket.SystemId!, 1f, 2f)!;
        var outside = harness.Provider.CreateAuthoredSite("field", "elsewhere", "other-system", 3f, 4f)!;
        ...
        Assert.Null(harness.Provider.GetAuthoredSite("field", "in-pocket"));
        // ...its object is terminal and announced the transition; the unrelated site is untouched.
        Assert.Equal(AuthoredSystemReconstructionStatus.Dissolved, inside.State.Status);
        Assert.True(insideChanged);
        Assert.Same(outside, harness.Provider.GetAuthoredSite("field", "elsewhere"));
    }
    ...
    public void CombatSitesInsideThePocketRefuseDissolutionUntilTheInventoryCanProveAbsence()
    {
        ...
        Assert.Equal(AuthoredActionStatus.NotReady, notReady.Status);
        Assert.Single(harness.Coordinator.CaptureRows());
    ...
    public void DissolveDeltaVerificationAcceptsOnlyTheExactRemoval()
    {
        ...
        bool Verify(WorldMapIndex.Snapshot after) => WorldNativeAuthored.VerifyDissolveDelta(before, after, pocket, entrance, ParentOf);
