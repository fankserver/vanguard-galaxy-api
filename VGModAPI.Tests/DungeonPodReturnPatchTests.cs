...
public sealed class DungeonPodReturnPatchTests
{
    ...
    public void NativeShapedReturnAccountsForRosterAndPersistedOverflowThenRejectsReplay()
    {
        ...
        var data = new NativeObject(); data.Fields["resumePodPlayer"] = true; data.Fields["resumePodPhase"] = "Arrived"; pods.Loaded(data, saved.Id);
        ...
        var operation = new NativeObject(); operation.Fields["operationShip"] = ship; var origin = new object(); var faults = 0;
        DungeonPodReturnPatches.Observer = new(state, pods, native, _ => origin, _ => true, _ => operationId); DungeonPodReturnPatches.Report = _ => faults++;
        try
        {
