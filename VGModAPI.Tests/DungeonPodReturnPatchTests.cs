...
public sealed class DungeonPodReturnPatchTests
{
    ...
    public void DockedRefundComposesNativeReceiptHooksWithSaveReload(bool persistedOverflow, bool nativeFailure)
    {
        ...
        DungeonRefundPatches.Hooks = new(state, observer, _ =>
        {
            ...
            return state.TrackOperation(new(saved.Id, saved.LocationId, saved.DungeonId, saved.AttackerShipId, saved.DungeonType, saved.NativePhase, saved.Outcome, saved.MissionProtection, saved.TerminalProgress, saved.Autonomous, retired: retired));
        }, _ => operationId);
        try
        {
