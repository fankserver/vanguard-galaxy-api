...
/// </summary>
public sealed class AuthoredSiteDeltaTests
{
    ...
    public void ExactSingleSiteGrowthIsAccepted()
    {
        ...
        Assert.True(WorldNativeAuthoredSites.VerifySiteDelta(before, book.Read(), created, book.Host));
    }
    ...
    public void NoOpAdoptedForeignAndExtraGrowthAreRejected()
    {
        ...
        Assert.False(WorldNativeAuthoredSites.VerifySiteDelta(before, book.Read(), new object(), book.Host));
        // Adoption: an existing native cannot be claimed as the creation.
        var preExisting = book.Add(book.Host, "pre");
        var mid = book.Read();
        Assert.False(WorldNativeAuthoredSites.VerifySiteDelta(mid, mid, preExisting, book.Host));
        // Extra growth: two new POIs is not a site creation.
        var first = book.Add(book.Host, "a");
        book.Add(book.Host, "b");
        Assert.False(WorldNativeAuthoredSites.VerifySiteDelta(mid, book.Read(), first, book.Host));
    }
    ...
    public void RemovalsAndForeignSystemMembershipAreRejected()
    {
        ...
        var created = book.Add(book.Host, "site-1");
        Assert.False(WorldNativeAuthoredSites.VerifySiteDelta(before, book.Read(), created, book.Host));
        ...
        Assert.False(WorldNativeAuthoredSites.VerifySiteDelta(before2, book2.Read(), created2, book2.Host));
    }
}
