...
public sealed class CargoAuthorSessionTests
{
    ...
    private sealed class Fixture
    {
        ...
        internal DungeonStatus AttachStatus = DungeonStatus.StaleTarget;
        ...
        /// <summary>
        /// One consolidated dungeon service now fronts operations, panel, commands, tactics and
        /// settlement, so the fixture dispatches every one of those members from a single fake.
        /// </summary>
        internal IDungeonService Dungeons => Fake<IDungeonService>((name, args) =>
        {
            if (name == "get_Availability") return ServiceAvailability.Available;
            if (name == "GetOperations") return Seed;
            if (name == "GetTargets") return Targets;
            if (name == "GetOperation" || name == "GetTarget") return null;
            ...
            if (name == "RegisterAction")
            {
                ...
                Actions.Add(key, (Action<DungeonPanelSnapshot>)args[3]!);
                Presenters.Add(key, (Func<DungeonPanelSnapshot, DungeonPanelAction?>)args[2]!);
                return new Lease(() => { Actions.Remove(key); Presenters.Remove(key); });
            }
            if (name == "add_Changed") { Boarding.Add((Action<BoardingEvent>)args[0]!); return null; }
            if (name == "remove_Changed") { Boarding.Remove((Action<BoardingEvent>)args[0]!); return null; }
            if (name == "add_SettlementChanged") { SettlementLeases++; Settlements.Add((Action<DungeonSettlementSnapshot>)args[0]!); return null; }
            if (name == "remove_SettlementChanged") { SettlementLeases--; Settlements.Remove((Action<DungeonSettlementSnapshot>)args[0]!); return null; }
            ...
            return Fake<IDungeonProvider>((method, margs) =>
            {
                ...
                if (method == "Attach") { AttachCalls++; return new DungeonResult(AttachStatus, "attach", AttachStatus == DungeonStatus.Attached ? Guid.NewGuid() : null); }
                Assert.Equal("Dispose", method); ProviderDisposals++; return null;
            });
        });
        /// <summary>optional:false now means the renderer reports no contextual actions, since the
        /// panel is no longer a separately injectable service that can simply be absent.</summary>
        internal CargoAuthorSession Create(bool optional = true, bool required = true)
        {
            if (!optional) ContextualActions = false;
            return new("item", Life, required ? Dungeons : null, Logs.Add);
        }
