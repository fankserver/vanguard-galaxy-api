...
public sealed class InstalledAuthoredSiteBindingTests
{
    [Fact]
    public void AuthoredSiteMembersBindToInspectedShapes()
    {
        ...
        foreach (var spec in AuthoredSiteBindings.Members)
        {
        ...
        foreach (var spec in AuthoredSiteBindings.Methods)
        {
            var type = assembly.MainModule.GetType(spec.Type);
            Assert.NotNull(type);
        ...
        foreach (var name in new[] { AuthoredSiteBindings.Salvage, AuthoredSiteBindings.Mining, AuthoredSiteBindings.SalvageData, AuthoredSiteBindings.HazardFieldData })
            Assert.Contains(assembly.MainModule.GetType(name).Methods, m => m.IsConstructor && !m.HasParameters);
        Assert.Contains(assembly.MainModule.GetType(AuthoredSiteBindings.HazardName).Fields, f => f.Name == "DamageInRadius");
        Assert.Contains(assembly.MainModule.GetType(AuthoredSiteBindings.DamageType).Fields, f => f.Name == "Radiation");
        foreach (var member in new[] { "ResearchStation", "RelayStation", "IndustrialFacility" })
            Assert.Contains(assembly.MainModule.GetType(AuthoredSiteBindings.DungeonType).Fields, f => f.Name == member);
        var asteroidType = assembly.MainModule.GetType(AuthoredSiteBindings.AsteroidField);
        Assert.Contains(asteroidType.Methods, m => m.IsConstructor && m.Parameters.Select(p => p.ParameterType.FullName)
            .SequenceEqual(new[] { "System.Int32", "System.Single", "System.Single", AuthoredSiteBindings.OreSet, AuthoredSiteBindings.OreSet, "System.Single" }));
        var faction = assembly.MainModule.GetType(AuthoredSiteBindings.Faction);
        ...
        var random = assembly.MainModule.GetType(AuthoredSiteBindings.SeededRandom);
        ...
        var poiBase = assembly.MainModule.GetType(AuthoredSiteBindings.Poi);
