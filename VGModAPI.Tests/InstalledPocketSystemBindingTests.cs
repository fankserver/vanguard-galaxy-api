...
public sealed class InstalledPocketSystemBindingTests
{
    [Fact]
    public void PocketSystemMembersBindToInspectedShapes()
    {
        ...
        foreach (var spec in PocketSystemBindings.Members)
        {
        ...
        foreach (var spec in PocketSystemBindings.Methods)
        {
            var type = assembly.MainModule.GetType(spec.Type);
            Assert.NotNull(type);
        ...
        var create = assembly.MainModule.GetType(PocketSystemBindings.Sandbox).Methods.Single(m => m.Name == "AddSideContentSystemToSystem");
        Assert.True(create.IsStatic && create.HasBody);
        var entrance = assembly.MainModule.GetType(PocketSystemBindings.System).Methods.Single(m => m.Name == "GetEntranceJumpgate");
        Assert.True(entrance.HasBody);
        var gate = assembly.MainModule.GetType(PocketSystemBindings.JumpGate);
