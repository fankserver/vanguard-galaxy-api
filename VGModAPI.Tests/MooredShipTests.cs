...
internal sealed class FakeMooredShipNative : IMooredShipNative
{
    ...
    private int _next;
    public string? CreateShip(Guid session, string stationPoiId, MooredShipDeclaration declaration)
    {
        Creations++;
    ...
    public void Maintain(Guid session, string stationPoiId, string unitId, MooredShipDeclaration declaration)
        => Maintained.Add((stationPoiId, unitId));
}

public sealed class MooredShipTests
{
    private sealed class Harness : IDisposable
    {
        internal readonly LifecycleHub Hub;
        internal readonly FakeMooredShipNative Native;
        internal readonly WorldDefinitionRegistry Combat;
        internal readonly MooredShipRegistry Ships;
        internal readonly MooredShipCoordinator Coordinator;
        ...
        internal Harness()
        {
            ...
            Native = new FakeMooredShipNative();
            ...
            Ships = new MooredShipRegistry(auth, Hub.CheckThread);
            Coordinator = new MooredShipCoordinator(Hub, Ships, Native, _ => true, (unit, key) => Protections.Add((unit, key)), _ => { });
    ...
    private static MooredShipDefinition Promise(int revision = 1)
    ...
    public void KeyedCreationOwnsIdentityProtectsAndMaintains()
    {
        ...
        Assert.Equal(WorldStatus.Succeeded, h.Provider.RegisterMooredShip(Promise()));
        Assert.Equal(WorldStatus.DuplicateDefinition, h.Provider.RegisterMooredShip(Promise()));
        h.BeginGameplay();
        ...
        Assert.Same(ship, h.Provider.CreateMooredShip("promise", "act3", "station-poi"));
        Assert.Same(ship, h.Provider.GetMooredShip("promise", "act3"));
    ...
    public void RestoredRowsReconstructAndSettleWithActualOutcomes()
    {
        ...
        Assert.Equal(WorldStatus.Succeeded, restored.Provider.RegisterMooredShip(Promise()));
    ...
    public void CrossKindKeysAndTypedRefusalsAreEnforced()
    {
        ...
        Assert.Equal(WorldContentStatus.Rejected, refused!.LastAction.Status);
        h.Native.RefuseCreation = false;
        Assert.Equal(WorldContentStatus.Rejected, h.Provider.CreateMooredShip("promise", "k", "station")!.LastAction.Status);
        Assert.Equal(1, h.Native.Creations);
        Assert.Null(h.Provider.GetMooredShip("promise", "k"));
        Assert.Null(h.Provider.CreateMooredShip("promise", "bad key\u0000", "station"));
    }
}
