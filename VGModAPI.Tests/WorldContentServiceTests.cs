...
public sealed class WorldContentServiceTests
{
    ...
    public void SequentialAuthenticationDoesNotDependOnReadinessAndContextLossClosesOperations()
    {
        ...
        Assert.Equal(WorldContentStatus.Succeeded, first!.RegisterCombatSite(definition));
        ...
        Assert.Equal(WorldContentStatus.Succeeded, second!.RegisterCombatSite(definition));
    ...
    public void PublicFacadeAuthenticatesCallerAndKeepsClosedNativeGateClosed()
    {
        ...
        Assert.Equal(WorldContentStatus.Succeeded, provider!.RegisterCombatSite(definition));
        Assert.Equal(WorldContentStatus.DuplicateDefinition, provider.RegisterCombatSite(definition));
        Assert.Equal(WorldContentStatus.InvalidDefinition, provider.RegisterCombatSite(new CombatSiteDefinition("Bad", 0, "Site", "player", 1)));
        Assert.Null(service.AcquireProvider(plugin));
        Assert.Equal(WorldContentStatus.Succeeded, provider.RegisterCombatSite(new CombatSiteDefinition("Migrate", 2, "New", "player", 1), new CombatSiteDefinition("Migrate", 1, "Old", "player", 1)));
        ...
        Assert.Equal(WorldContentStatus.InvalidDefinition, provider.RegisterCombatSite(new CombatSiteDefinition("BadMigration", 2, "New", "player", 2), new CombatSiteDefinition("BadMigration", 1, "Old", "player", 1)));
        ...
        service.Dispose();
        Assert.Equal(WorldContentStatus.UnknownProvider, provider.RegisterCombatSite(definition));
