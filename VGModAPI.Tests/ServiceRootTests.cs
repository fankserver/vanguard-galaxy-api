    ...
    private static ModServices Compose(LifecycleHub hub, ModInformationCatalog catalog)
    {
        ...
        return (ModServices)typeof(ModServices).GetConstructors(BindingFlags.NonPublic | BindingFlags.Instance)[0].Invoke(
            new object[] { lifecycle, mods, new PersistenceService(hub), missions, travel, station,
                new RecipeCatalogService(hub, null, _ => { }), new RecipeQuoteService(hub, null, _ => { }), jobs, commands, new HudService(hub, hub.ReportSubscriberFailure), new ForgeUiService(hub, null, hub.ReportSubscriberFailure), new BoardingRuleService(hub, hub.ReportSubscriberFailure), new BoardingCombatService(hub, hub.ReportSubscriberFailure), new DungeonRewardService(hub, hub.ReportSubscriberFailure), new BoardingCommandService(hub, null, null, () => false) });
    }
