...
The internal pod persistence/codec/adapter primitives are **not wired into live hooks** and do not currently provide native recovery. They retain independent pod and occurrence identities, the exact parent ship identity, phase, a copied return manifest, and separate attempted/delivered flags. A known-empty manifest is distinct from missing state. A potentially partial return attempt is not automatically retried or described as successful delivery.
...
These primitives have host coverage only. Complete reconstruction, operation ownership, pending directives, terminal settlement continuity and native acceptance remain required before automatic encounter recovery can be claimed. Native and sidecar files are not an atomic transaction; no cross-file crash atomicity is promised.
