...
`-DungeonConsumerManifest <private.json>` adds the actual BoardAlways and DungeonAuthor plugins to the isolated panel phase (it implies `-DungeonPanelProbe`). The schema-1 manifest has `rewardItemId` (a verified native item identifier) and exactly two `binaries` records, each with `name`, absolute `path`, lowercase SHA-256 `sha256` and 40-hex source `revision`: `VGBBoardAlways.dll`, `DungeonAuthor.dll`. The author's project links the `AuthoredDungeon` sources into its own DLL; do not stage the separate example-library output. Build all against API 0.2.0; preparation checks assembly identities, references and hard API dependencies. It pins BoardAlways enabled with difficulty 2 and integrity 1.5. The source manifest, copied DLLs and generated configs are revalidated at Run. A real pointer activates the separately loaded author's attach action twice. The runner records only `INPUTS_SENT`; post-exit verification additionally requires consumer load/registration logs and exactly one `Attached` then duplicate `TargetInUse` result. The v3 consumer phase also uses the public command service to acquire exclusive control, start a manual installation approach, refuse a competing owner and duplicate start, admit options and resume the same operation, reject simulation-only commands before simulation, then cancel before any native approach tick. A separate exact `dungeon-commands-v1` receipt requires observed retirement and unchanged donor identity, crew inventory and docking state. Read-only native inventory inspection selects one existing crew member; all mutations use typed public commands, with explicit faction consent and finally-based cancellation/disposal. After the pre-approach cancellation it starts a second manual operation with six existing crew, permits real installation approach/walking, requires observed active airlock arrival, requests retreat and waits for observed returning-crew settlement and operation retirement. The separate `dungeon-walk-v1` receipt reconciles the same donor's inventory against observed casualties and reconciles reported prisoner deliveries against the same donor's brig while rejecting unexpected capture. Failure-path cleanup requests are diagnostic only, never settlement evidence. A PASS is revision-scoped and requires an exact active-roster debit of the requested manifest followed by final reconciliation; initial arrival alone is not settlement. It does not qualify applied option effects, live rule effects, victory, reward delivery or persistence. The generated target is not added to POI save data; no save/load claim is made. The optional manifest `mode` is `Retreat` (default), `Combat`, `Reward` or `SaveLoad`. Combat mode is separately pinned by a validated marker: it retains exact crew-debit/return checks, enables automatic movement through public options, requires native victory and scoped health/power callback invocation, clicks the actual author's leave-shipment choice, and requests/confirms extraction through public tactics. Its receipt cannot substitute for the retreat receipt. These policy checks establish invocation and scope filtering, not a numerical damage/health calibration. Choosing leave does not qualify reward delivery. Reward mode requires `Titanium Plate` and replaces six Deckhands with six Marines in the copied-save fixture before the crew accounting baseline (requiring no existing Marines and preserving total crew); this is native fixture construction, not API crew provisioning. It chooses the actual author's recover action, registers a public loot multiplier of two, and requires both four observed inventory-delivery units and a four-item increase in the same donor's cargo. Its separate reward receipt is mandatory; collection alone is not delivery. This does not cover overflow, data/credit routing, mission rewards or save/load. SaveLoad mode repeats the retreat panel phase, then creates a separate probe-owned installation, starts it and places its native location in the POI's saved persistable list as fixture setup. It saves an active checkpoint and a save-as copy, switches to an unrelated slot that must contain no occurrence, and reloads the copy in-process; native loading must reconstruct the target and operation without recreating a scene proxy. The probe requires stable occurrence identity, new session/target/control identities, no second crew debit, and settled crew return after resuming control. While active it exercises an ephemeral skipped save and exhausted metadata-write retries, requiring unchanged live state and successful-checkpoint bytes. Failure injection is confined to a directory at a sandbox metadata filename; a failed write may leave partial data and is never loaded. After return it overwrites and reloads the active checkpoint as resolved to reject duplicate operation/crew recovery, then restores the earlier save/meta bytes at those same paths and verifies an independent return. It then disposes the provider registration before another active reload, keeping it absent through occupied active-operation reconstruction. Reacquisition must expose the same retained occurrence, refuse missing definitions and newer versions without invoking incompatible behavior, then permit matching-version behavior exactly once and settle crew return. This tests warm registration absence and explicit no-implicit-migration semantics, not plugin removal/cold-process recovery, unsupported schema conversion or cross-file atomicity. Private manifests and raw logs must not be published.

`-DungeonPanelProbe` requires at least 900 seconds, the default windowed resolution and a settled POI with no other dungeon location appearing during the phase. It includes readiness and then constructs one scene-local native dungeon location using an installed definition. It does not add that target to save data or start combat. It registers a long status section and enabled/disabled actions, opens the native panel, sends a pointer click through a temporary Unity input device, checks close/reopen view identity, destroys its generated target and requires stale-target refusal. All contributor leases and the input device are released. A separate mandatory panel receipt prevents a readiness-only run from passing this phase. The private `dungeon-panel-actions.png` capture and matching SHA-256 record are mandatory and launcher-verified for existence, bounded size and integrity. The capture follows pointer readiness; inspect it rather than treating its presence as full visual acceptance. The v4 receipt also requires offscreen selection to reveal a tail action below the long section, a real click there, the 1.25× native-panel scale compact drawer open/close path, and a working click after original scale restoration. `dungeon-panel-scrolled` and `dungeon-panel-scaled` PNG/SHA pairs are mandatory alongside the original capture. Selection is explicitly seeded for scrolling, not navigated from an initial native control. The receipt additionally requires Down/Enter keyboard navigation/submission and D-pad Up/South controller navigation/submission through the installed input module, using temporary devices and an explicitly seeded initial selection. It does not prove unseeded focus discovery, wheel scrolling or every control scheme. The receipt requires real enabled/disabled pointer clicks and independent dispatch for two contributor identities. Cached UnityEvent callbacks (deliberately invoked directly, not presented as pointer input) test fresh eligibility revalidation, disposed-registration refusal and closed/previous-view refusal. Both identities are owned by the runner; this does not substitute for combined-consumer acceptance. This generated-location subset is not ship-panel, full scaling, full keyboard/controller, authored-choice, combat, settlement or recovery acceptance.
...
`-BlueprintPinProbe -BlueprintPinBin <directory> -BlueprintPinRevision <40-hex-commit> -BlueprintPinSha256 <64-hex-digest>` selects a separate read-plus-consumer and controlled crafting phase. Supply a reviewed Blueprint Pin 0.3.0 build with its typed API 0.2.0 dependency; the launcher checks assembly dependencies and binary identity, copies that DLL and the built `ForgeInspectorHost`/`ForgeInspector` example assemblies, and enables recipe/HUD integration and crafting commands in the sandbox. The probe drives the real Pin action, checks a one-batch target, navigates away and back through the HUD to the exact variant, and closes the panel. It also finds a single Forge producer and navigates through an ingredient click, then checks an alternative-producer chooser, route-specific enabled states, a disabled non-Forge-row pointer click after programmatically scrolling to the final row, and backing out without clearing the pin. Fixture setup selects one batch through the native slider; native recipe selection otherwise defaults to the maximum craftable quantity. The fixture must contain eligible inputs (up to seven ingredient rows, two to 32 alternative routes ending in a non-Forge route). A final phase pins two future batches of an affordable unoccupied multi-unit Forge recipe, queues through public commands, drives one real native completion, cancels the remainder, then queues and completes a replacement batch. The completed target remains visible at zero until the probe closes it through the real close button. It verifies actual inventory deltas against delivery observations and HUD allocation/remaining counts, distinguishing batches from output units. Temporary runner-only progression patches pause background Forge/refinery scheduling while individual native job methods are driven; patches are removed in `finally`. This does not qualify natural timing. Four screenshots, including partial and cancelled pin states, are hash-bound. The consumer itself remains patch-free. A save/reload phase then proves that saving preserves the current pin, reloading clears it without restoring consumer state, and the same consumer can pin again in the new Forge view. Save-as followed by a slot switch and return must also leave no pin; old station handles are refused and the exact recipe remains reopenable. Loading-overlay destruction is required before renewed input. Build the independent host with `make build-forge-host` and include its output in the build root. Before the final co-load phase, two temporary Forge selection subscribers are registered in order. The first deliberately throws once on a real slider change; the second must still receive that change. Both are removed and the batch count restored before real-consumer interactions. A mandatory bounded `forge-callback-isolation.txt` receipt records success; this tests selection-listener isolation, not faults in every service. The final co-load phase physically invokes its Inspect action, opens both consumers' independently owned panels, uses Inspector's exact-recipe navigation without clearing the pin, programmatically scrolls the shared horizontal panel viewport, and closes each panel independently. A fifth hash-bound `forge-consumers` capture is mandatory. A generated-equipment phase then selects a single-template recipe whose missing item ingredients can be made by at most eight direct Forge producers using available refined materials (at most 512 batches each). Planning reserves shared raw stock for all preparation jobs and the equipment job together. Preparation uses public queue commands and native per-batch completion, never inserted inventory; command admission and the final refreshed quote must still succeed. It then queues one equipment batch through public commands and drives its native completion. The quoted level must match the native job; verified item level, rarity and quantity must match the recipe, and delivery tuples must reconcile all native inventory deltas. This case runs with cargo delivery disabled and enabled through public settings commands, requiring actual Armory and ShipCargo destinations respectively. The original preference is restored in `finally` and verified afterwards. The mandatory bounded `forge-generated.txt` and `forge-routing.txt` receipts record both runs and restoration, not Industry stations, skill changes or every equipment builder. A third generated batch exercises the native bonus branch under a synchronous, disposable skill fixture: the existing Industrial tree's bonus node entry is temporarily replaced and its increase tuned to probability one. The original entry (including absence) and tuning are restored in `finally`, before any frame yield. Two verified generated deliveries must belong to one batch, reconcile inventory, and pass the bounded `forge-bonus.txt` receipt. This does not qualify earned skill progression or natural bonus rates. A fourth generated job is admitted before temporarily setting ship cargo capacity to zero for its synchronous native completion. Native fullness and actual Armory fallback are required while cargo preference remains enabled; capacity is restored in `finally` and checked before the mandatory `forge-cargo-full.txt` receipt. This tests a controlled capacity change between admission and delivery, not physically filling cargo with inventory. Finally, a missing vanilla recipe identifier must be refused without changing the selection. The native station exit method is then driven (not a physical pointer click); real Undocking/Leaving facts and completed undocking are required before checking that Forge selection/actions disappear and exact-recipe navigation returns NotAtStation. A mandatory bounded `forge-missing-context.txt` receipt records these checks. This does not qualify activating a producer from the chooser, plural distinct output resources, failed saves/loads, rollback, other callback fault paths or arbitrary mod combinations. Other probes and consumers are excluded.
