...
Requires API 0.1.25. Enable `[Persistence] Enabled = true` and `[Bars] Enabled = true`. Bars are opt-in; check `ModApi.Bars` and the `owned-bars` capability rather than assuming availability. `RuntimeQualified` remains false: bounded native core checks do not establish complete consumer-combination acceptance.
...
Foundation/Anima/TTS combination behavior, missing author binaries, consumer-authored dialogue, natural daily-time transitions and native linked-story restoration remain outside that bounded evidence. See [probe scope](../tools/bar-probe-scope.md) and [independent authors](../examples/OwnedBarAuthor/README.md).
