...
Member mappings apply to the original `Assembly-CSharp.dll` SHA-256 `71636b8fcfde62e2e00ab64c417e878d1a24d433f1bac10e1ddc8750193a35c2`. Original game source remains private. Unknown hashes cannot enable integration by matching names/signatures alone. Consult [compatibility](compatibility.md) and [lifecycle](lifecycle-contract.md) for threading, patch-group rollback and readiness constraints.
...
- Resume/reconstruction resolves the current player ship rather than restoring an attacking-occurrence identity from `parentShipId`.
