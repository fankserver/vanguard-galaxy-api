...
`GetJumpCounts(session, fromSystemId)` reads the graph once for a complete distance list (reachable IDs only). `GetJumpCount(session, fromSystemId, toSystemId)` computes the shortest **unweighted, directed jump-gate hop count**. It uses existing gate targets, ignores access passes, hostility, fuel and travel restrictions, and is not an access-aware route or promise that the player can travel. A gated edge still counts as one hop. Same-system count is zero. Missing identifiers and disconnected systems have distinct statuses; unsuccessful results have no hop count.
