...
The optional persistence service associates mod state with exact vanilla save snapshots. It does not automatically migrate existing consumer sidecars. Runtime session IDs are never persistence keys.
...
The identity hashes the complete `.save` file only, not its companion version-metadata `.meta` file. Both writes are still required for a successful lifecycle save outcome. Metadata is not treated as a snapshot discriminator. Neither inspected wrapper contains a unique-per-save identifier; the displayed timestamp is filesystem metadata. See [schema constraints](persistence-schema.md). No timestamp uniqueness is assumed.

This has an unavoidable limitation: two different mod states can accompany byte-identical vanilla files. Without an identifier embedded in vanilla, a later rollback to identical bytes cannot be distinguished. Therefore an existing slot+vanilla-hash association MUST NOT be replaced with different mod state or campaign identity. Reject the conflicting commit, retain the original association and diagnostics, and pause affected persistence. When conflict evidence is successfully persisted, the coordinator/store blocks subsequent exact-hash restoration until explicit recovery. Marker-write failures remain subject to the explicit durability limits in the storage document. This also refuses legitimate same-campaign forward progression when mod state changes but the `.save` bytes do not: the newer mod state is not committed, and persistence pauses rather than silently dropping the conflict. This availability cost is intentional. Do not claim this solves arbitrary identical-byte rollback. Embedding identity into vanilla files is outside this contract.
...
Retaining generations costs disk space. There is no automatic pruning: explicit deletion loses recovery state. A vanilla write and generation publication are not a cross-file transaction. The [coordinator](persistence-storage.md) stages provider snapshots, publishes only after matching successful writes and diagnoses incomplete commits.
