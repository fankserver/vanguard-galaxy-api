...
Persistent world content the API declares, persists and reconstructs for a mod is **owned content** (matching the `OwnedItem*`/`OwnedRecipe*` families and CONTRIBUTING's *API-owned content*). The old `Resource*` prefix is retired because it duplicated the `Owned*` family, was not game terminology, and was applied inconsistently (combat sites never used it). The world-content occurrence kinds are named by their game concept: `PocketSystem`, `WormholePair`, `ResourceSite`, `MooredShip`, `CombatSite`. Shared, kind-independent status/reason types are named neutrally: `ReconstructionStatus`, `ReconstructionFailureReason`, `WorldContentStatus`, `WorldContentResult`, `RemovalStatus`. `WorldContentStatus` is the single outcome axis for both declarations (`Register*`) and runtime actions on an owned poi; `ReconstructionStatus` (+ its reason) is the lifecycle/state axis; `RemovalStatus` is a separate removal-readiness report. No further status vocabulary is introduced — the game itself has no general status/result enum, so these stay at the minimum the API needs. Author-local keys remain `localId`/`poiKey` for world content (the key names a placed authored POI, not a native or instance GUID).
