...
The bottom action bar contains **Settings** when the selected mod has explicitly published player-facing settings, plus one **Check for updates** button for all supported mods, subject to cooldown/backoff; there is no separate retry button. Settings use the separate [mod settings contract](mod-settings.md); the inventory never reflects over arbitrary plugin configuration. Failures are labeled **Update check failed**. **Download update** appears in the same bottom bar only when an update is available; it opens a browser page, not an installer. Links are validated again on explicit activation. No remote images, startup popup, mod toggles, downloads, installations or save operations.
