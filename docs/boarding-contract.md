...
- Encounter tuning multiplies defender power and health only during new native simulation creation, before defender placement. Both easier and harder settings are supported. Saved native multipliers remain authoritative and are not reapplied on reconstruction. Changes to rules do not retune existing simulations. Native pre-entry estimates incorporate the power multiplier; like native live estimates, they are power heuristics, not health-aware combat forecasts. Zero health produces zero estimated defender power; rounding and individual crew composition still limit prediction accuracy.
