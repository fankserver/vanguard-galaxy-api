...
The pilot is phased: it exercises the deterministic cases the supplied sandbox fixtures can support and records the rest as NOT-RUN matrix cells in the receipt — it never fabricates an arrival/dock or an unconditional PASS. Bounded per-case waits with diagnostic actual state on failure; a genuinely failed native case fails the whole run (it is never hidden). Receipts: `travel-station.txt` (first line PASS/FAIL + per-case summary), `travel-station-receipt.tsv` (per-case status/native identity), and `travel-station-events.tsv` (observed public-event trace). `travel-station.txt` starts with PASS only when all attempted cases passed; NOT-RUN cells are reported alongside.

Attempted cases: initial placement is not an arrival; genuine in-system Requested->Departed->Arrived->RouteCompleted via native `SetRouteToPOI`; chained waypoints (RouteCompleted only at the final boundary); early cancel at a known origin (Cancelled(origin), no Departed/RecoveredPlacement); real `DockingOption.Undock` -> Undocking/Leaving (nested `ResetDockingOption` null ship handled). Cells the standard fixtures cannot deterministically exercise (empty-origin reroute after unload, jumpgate/wormhole charging, same/different-size reinit suppression, stale-old-session coroutine replay) are recorded as NOT-RUN with the reason rather than deferred to an unconditionally-passing path; an owner-driven fixture can unlock them in a later phase.
