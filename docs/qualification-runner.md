...
qa-09 also passed synchronous wizard configuration, mining-space save/load, autosave rotation and successful retry recovery. Still separate: empty-space/in-transit loads, explicit delayed stale-callback injection, unsupported game-hash behavior, pointer-driven UI acceptance, and multi-mod combinations. The inspected autosave selector chooses the first missing slot, then the oldest file by modification time; the four-call 0/1/2/0 expectation assumes distinct timestamps on the tested filesystem and may fail on coarse-resolution filesystems. Do not set RuntimeQualified merely because the automated subset passes.
...
qa-16 caught an obsolete journal acceptance binding; qa-17 passed 24 checks after its repair. Original file hashes/preferences were independently verified. Later consumer hardening and negative startup checks still require latest-build evidence before the pilot PRs merge. This tooling does not qualify all mission domain hooks, TravelJournal coexistence or the rest of milestone 01.
