...
Known contract limits and failure handling are specified in [lifecycle-contract.md](lifecycle-contract.md). Persistence coordination is opt-in and experimental. Controlled MissionJournal/Stockpile coordinated pilots now cover queue/history roundtrips, transfer economics, shared write-refusal/retry, protected imports and teardown; full owner acceptance and unexercised recovery scenarios remain separate. Mission observation is an opt-in experimental integration; see [its contract and remaining limits](mission-events.md). Story/UI capabilities are not implemented yet.
