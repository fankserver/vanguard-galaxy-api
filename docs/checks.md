...
The installed-CONSUMER metadata checks of the actual-consumer qualification probe need a locally built consumer binary, so they are excluded from `make test` and from `check-local` and run separately:
