...
On the inspected assembly, one **Mods** button joins the actual main menu. The button caption is centered like the native menu entries. Opening it refreshes the local catalog and shows mod names, installed versions, author descriptions and update status. Technical capability details are not part of this player-facing screen. Scroll the list/details or use Previous/Next to browse; Close or Escape dismisses it. Names are bounded in rows, with installed versions in a separate column and full bounded names in details. The list creates only viewport-sized pooled rows, not 4096 Unity objects. The validated ASCII HTTPS destination is a separate plain-text element above author details; only activating **Open project** calls the browser, with the link revalidated at that moment. No remote images, startup popup, mod toggles, downloads, installations or save operations. **Check for updates** requests a refresh immediately, subject to cooldown/backoff. **Open download page** appears when an update is available; it opens a browser, not an installer.
...
Host tests cover bounded/stale inventory, explicit links and lifetime policies. Installed metadata checks pin reflected members and compile-only references. Controlled native acceptance covers two real consumers, metadata failures, new/load/menu transitions, virtual keyboard/mouse input, scrolling, modal coexistence, canvas replacement, resizing/scaling and shutdown. Update acceptance combines controlled service scheduling/failure/cache cases with actual platform TLS, redirect, DNS, certificate-rejection and cancellation checks. An explicit release action is verified in the default Chrome browser without downloading or installing a package. These bounded cases do not establish acceptance of the current simplified controls, physical gamepad behavior, other browsers, universal runtime qualification or save compatibility.

The package includes `vgmodapi.vgmod.json` beside the API DLL, providing the official project link and description. Missing author metadata is not a load error. The declared optional qualification-guard dependency is loader ordering metadata, not evidence that a guard DLL is installed; declared dependencies are available through the catalog API.
...
Successful data is cached outside saves, in BepInEx's cache directory under `VGModAPI-updates-v1`. SHA-256 context keys include plugin ID, exact configured feed URL, schema and channel, but not installed version: cached versions are compared with the current loader version. At most 128 entries of at most 16 KiB plus a timestamp are retained; entries older than 30 days, future-dated, malformed or context-mismatched are ignored. Cache I/O runs in the background and failures are non-fatal. The screen shows a concise update status. Failure never means up to date; installed-ahead results never recommend downgrade. Release destinations are shown separately as ASCII HTTPS hosts and open only through **Open download page**, with URL validation repeated on activation.

Host fake-transport tests cover the parser, transport, scheduler, cache and immediate manual requests. Native TLS/runtime and complete updated-control acceptance remain part of #80; host tests alone do not qualify them.
