...
Host tests cover bounded/stale inventory, explicit links and lifetime policies. Installed metadata checks pin reflected members and compile-only references. Controlled native acceptance covers two real consumers, metadata failures, new/load/menu transitions, virtual keyboard/mouse input, scrolling, modal coexistence, canvas replacement, resizing/scaling, unavailable diagnostics and shutdown. Update acceptance combines controlled service scheduling/failure/cache cases with actual platform TLS, redirect, DNS, certificate-rejection and cancellation checks. An explicit release action is verified in the default Chrome browser without downloading or installing a package. These bounded cases do not establish physical gamepad behavior, other browsers, universal runtime qualification or save compatibility.
