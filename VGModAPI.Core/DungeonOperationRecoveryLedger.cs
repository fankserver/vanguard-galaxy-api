...
internal sealed class DungeonOperationRecoveryLedger
{
    ...
    internal void Track(DungeonOperationResumeState next)
    {
        ...
        {
            if (previous.LocationId != next.LocationId || previous.DungeonId != next.DungeonId || previous.AttackerShipId != next.AttackerShipId || previous.DungeonType != next.DungeonType || previous.Autonomous != next.Autonomous)
