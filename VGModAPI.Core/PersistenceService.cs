...
internal sealed class PersistenceService : IPersistenceApi, IDisposable
{
    ...
    internal string CanonicalLoadPath(string path) => _coordinator.CanonicalLoadPath(path);

    internal WorldGenerationReader CreateWorldReader() => _coordinator.CreateWorldReader();
