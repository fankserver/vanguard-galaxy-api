...
internal sealed class AuthoredSystemRegistry : IDisposable
{
    internal sealed class Provider : IDisposable
    {
        private readonly AuthoredSystemRegistry _registry;
        internal string Owner { get; }
        internal Provider(AuthoredSystemRegistry registry, string owner) { _registry = registry; Owner = owner; }
        internal bool Register(AuthoredSystemDefinition definition, AuthoredSystemDefinition? previous = null)
        {
    ...
    private readonly Dictionary<(string Owner, string Local), AuthoredSystemDeclaration> _definitions = new();
    private readonly Dictionary<(string Owner, string Local), AuthoredSystemDeclaration> _previous = new();
    private long _revision;
    private bool _disposed;
    internal long Revision { get { _checkThread(); return _revision; } }
    internal AuthoredSystemRegistry(StoryHostAuthenticator authenticate, Action checkThread)
    ...
    internal bool Register(Provider provider, AuthoredSystemDefinition definition, AuthoredSystemDefinition? previous = null)
        => Register(provider, Map(definition ?? throw new ArgumentNullException(nameof(definition))), previous == null ? null : Map(previous));
    private static AuthoredSystemDeclaration Map(AuthoredSystemDefinition definition)
        => new(definition.LocalId, definition.Revision, definition.Name);
    private bool Register(Provider provider, AuthoredSystemDeclaration definition, AuthoredSystemDeclaration? previous)
    {
        _checkThread();
    ...
    internal bool TryResolve(Provider provider, string localId, out AuthoredSystemDeclaration? definition)
    {
        _checkThread();
        definition = null;
