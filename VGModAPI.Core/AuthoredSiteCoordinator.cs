...
internal sealed class AuthoredSiteRegistry : IDisposable
{
    internal sealed class Provider : IDisposable
    {
        private readonly AuthoredSiteRegistry _registry;
        internal string Owner { get; }
        internal Provider(AuthoredSiteRegistry registry, string owner) { _registry = registry; Owner = owner; }
    ...
    private readonly Dictionary<(string Owner, string Local), AuthoredSiteDeclaration> _definitions = new();
    private readonly Dictionary<(string Owner, string Local), AuthoredSiteDeclaration> _previous = new();
    private bool _disposed;
    internal AuthoredSiteRegistry(StoryHostAuthenticator authenticate, Action checkThread)
    ...
    internal bool Register(Provider provider, AuthoredSiteDeclaration definition, AuthoredSiteDeclaration? previous)
    {
        _checkThread();
    ...
    internal bool TryResolve(Provider provider, string localId, out AuthoredSiteDeclaration? definition)
    {
...
internal interface IAuthoredSiteNative
{
    ...
    string? CreateSite(Guid session, string systemId, float x, float y, AuthoredSiteDeclaration declaration);
    ...
    string? ResolveSite(Guid session, string systemId, string poiId, AuthoredSiteKind kind);
...
/// </summary>
internal sealed class AuthoredSiteCoordinator : IDisposable
{
    private readonly LifecycleHub _hub;
    private readonly AuthoredSiteRegistry _definitions;
    private readonly IAuthoredSiteNative _native;
    private readonly Func<Guid, bool> _persistenceReady;
    private readonly Action<Exception> _report;
    private readonly IDisposable _subscription;
    private readonly Dictionary<(string Owner, string Local, string Key), AuthoredSiteOccurrence> _committed = new();
    ...
    internal AuthoredSiteCoordinator(LifecycleHub hub, AuthoredSiteRegistry definitions, IAuthoredSiteNative native,
    ...
    internal void RestoreRows(Guid session, AuthoredSiteOccurrence[] rows)
    {
        _hub.CheckThread();
    ...
    internal AuthoredSiteOccurrence[] CaptureRows() { _hub.CheckThread(); return _committed.Values.ToArray(); }

    internal IReadOnlyList<AuthoredSiteOccurrence> Occurrences(string owner)
    {
        _hub.CheckThread();
        return _disposed ? Array.Empty<AuthoredSiteOccurrence>() : _committed.Values.Where(o => o.Owner == owner).ToArray();
    }
    internal AuthoredSiteOccurrence? TryGetOccurrence(string owner, string localId, string occurrenceKey)
    {
        _hub.CheckThread();
        ...
        Guid expectedSession, string localId, string occurrenceKey, string systemId, float x, float y)
    {
        ...
        try
        {
            ...
            var occurrence = new AuthoredSiteOccurrence(provider.Owner, localId, occurrenceKey, declaration.Revision,
    ...
    internal AuthoredSiteState ReconstructionState(string owner, string localId, string occurrenceKey)
    {
        _hub.CheckThread();
        if (_disposed) return new AuthoredSiteState(AuthoredSystemReconstructionStatus.Pending);
        if (_failed.Contains((owner, localId, occurrenceKey)))
            return new AuthoredSiteState(AuthoredSystemReconstructionStatus.Failed, AuthoredSystemFailureReason.NativeMissing);
        return _committed.TryGetValue((owner, localId, occurrenceKey), out var row)
            ? Resolve(row) : new AuthoredSiteState(AuthoredSystemReconstructionStatus.Pending);
    }
    ...
    internal AuthoredSiteState Resolve(AuthoredSiteOccurrence occurrence)
    {
        ...
        if (!_definitions.TryResolveMigration(occurrence.Owner, occurrence.LocalId, out var liveRevision, out var previousRevision))
            return new AuthoredSiteState(AuthoredSystemReconstructionStatus.Failed, AuthoredSystemFailureReason.MissingDefinition);
        if (liveRevision != occurrence.Revision)
        {
            ...
            else return new AuthoredSiteState(AuthoredSystemReconstructionStatus.Failed, AuthoredSystemFailureReason.RevisionMismatch);
        }
        if (!_persistenceReady(Session()))
            return new AuthoredSiteState(AuthoredSystemReconstructionStatus.Failed, AuthoredSystemFailureReason.PersistenceUnavailable);
        try
        {
            if (_native.AmbiguousCount(Session(), occurrence.PoiId) > 1)
                return new AuthoredSiteState(AuthoredSystemReconstructionStatus.Failed, AuthoredSystemFailureReason.AmbiguousIdentity);
            var poiId = _native.ResolveSite(Session(), occurrence.SystemId, occurrence.PoiId, occurrence.Kind);
            return poiId == null
                ? new AuthoredSiteState(AuthoredSystemReconstructionStatus.Pending)
                : new AuthoredSiteState(AuthoredSystemReconstructionStatus.Reconstructed, poiId: poiId);
        }
        catch (Exception error)
        { _report(error); return new AuthoredSiteState(AuthoredSystemReconstructionStatus.Failed, AuthoredSystemFailureReason.NativeMissing); }
    }
