...
/// </summary>
internal sealed class WorldNativeAuthoredSites : IAuthoredSiteNative
{
    ...
    internal WorldNativeAuthoredSites(GameAdapter game, Assembly assembly, Action<Exception>? report = null)
    {
        ...
        var resolved = AuthoredSiteBindings.Validate(assembly);
        ...
        _salvageType = Get(AuthoredSiteBindings.Salvage); _miningType = Get(AuthoredSiteBindings.Mining);
        _salvagePoi = Ctor(_salvageType); _miningPoi = Ctor(_miningType);
        _salvageData = Ctor(Get(AuthoredSiteBindings.SalvageData));
        _hazardFieldData = Ctor(Get(AuthoredSiteBindings.HazardFieldData));
        _hazardEnum = Get(AuthoredSiteBindings.HazardName); _damageEnum = Get(AuthoredSiteBindings.DamageType);
        _dungeonEnum = Get(AuthoredSiteBindings.DungeonType);
        var field = Get(AuthoredSiteBindings.AsteroidField);
        _asteroidField = field.GetConstructor(new[] { typeof(int), typeof(float), typeof(float), Get(AuthoredSiteBindings.OreSet), Get(AuthoredSiteBindings.OreSet), typeof(float) })
            ?? throw new MissingMethodException(field.FullName, ".ctor");
        var system = Get(AuthoredSiteBindings.System);
        ...
        _elementFaction = Get(AuthoredSiteBindings.Element).GetProperty("faction", BindingFlags.Public | BindingFlags.Instance)
            ?? throw new MissingMemberException("MapElement.faction");
        var poi = Get(AuthoredSiteBindings.Poi);
        _hazardField = Field(poi, "hazardFieldData"); _asteroidsInitialized = Field(poi, "asteroidsInitialized");
        var data = Get(AuthoredSiteBindings.SalvageData);
        ...
        var hazard = Get(AuthoredSiteBindings.HazardFieldData);
        ...
        _factionType = Get(AuthoredSiteBindings.Faction);
    ...
    public string? CreateSite(Guid session, string systemId, float x, float y, AuthoredSiteDeclaration declaration)
    {
        try
        {
            ...
            object? faction = null;
            if (declaration.Kind == AuthoredSiteKind.SalvageSite)
            {
    ...
    private void PopulateSalvage(object host, object poi, AuthoredSiteDeclaration declaration)
    {
    ...
    private void PopulateMining(object host, object poi, AuthoredSiteDeclaration declaration)
    ...
    public string? ResolveSite(Guid session, string systemId, string poiId, AuthoredSiteKind kind)
    {
        ...
        if (host == null) return null;
        var expected = kind == AuthoredSiteKind.SalvageSite ? _salvageType : _miningType;
