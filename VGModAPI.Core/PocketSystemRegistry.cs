...
internal sealed class PocketSystemRegistry : IDisposable
{
    ...
    {
        private readonly PocketSystemRegistry _registry;
        internal string Owner { get; }
        internal Provider(PocketSystemRegistry registry, string owner) { _registry = registry; Owner = owner; }
        internal bool Register(PocketSystemDefinition definition, PocketSystemDefinition? previous = null)
        {
    ...
    private readonly Dictionary<(string Owner, string Local), PocketSystemDeclaration> _definitions = new();
    private readonly Dictionary<(string Owner, string Local), PocketSystemDeclaration> _previous = new();
    private long _revision;
    private bool _disposed;
    internal long Revision { get { _checkThread(); return _revision; } }
    internal PocketSystemRegistry(StoryHostAuthenticator authenticate, Action checkThread)
    ...
    private static PocketSystemDeclaration Map(PocketSystemDefinition definition)
        => new(definition.LocalId, definition.Revision, definition.Name, definition.Placement, definition.FactionId, definition.SectorName, definition.Quiet);
    ...
    internal bool TryResolve(Provider provider, string localId, out PocketSystemDeclaration? definition)
    {
        _checkThread();
        definition = null;
