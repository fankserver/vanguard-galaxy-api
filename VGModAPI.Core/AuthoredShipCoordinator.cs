...
internal sealed class AuthoredShipDeclaration
{
    ...
    internal AuthoredShipDeclaration(AuthoredShipDefinition definition)
    {
    ...
    internal AuthoredShipDefinition ToDefinition()
...
internal sealed class AuthoredShipOccurrence
{
    ...
    internal AuthoredShipOccurrence(string owner, string localId, string occurrenceKey, int revision, string stationPoiId, string unitId)
    {
...
internal sealed class AuthoredShipRegistry : IDisposable
{
    internal sealed class Provider : IDisposable
    {
        private readonly AuthoredShipRegistry _registry;
        internal string Owner { get; }
        internal Provider(AuthoredShipRegistry registry, string owner) { _registry = registry; Owner = owner; }
    ...
    private readonly Dictionary<(string Owner, string Local), AuthoredShipDeclaration> _definitions = new();
    private readonly Dictionary<(string Owner, string Local), AuthoredShipDeclaration> _previous = new();
    private bool _disposed;
    internal AuthoredShipRegistry(StoryHostAuthenticator authenticate, Action checkThread)
    ...
    internal bool Register(Provider provider, AuthoredShipDeclaration definition, AuthoredShipDeclaration? previous)
    {
        _checkThread();
    ...
    internal bool TryResolve(Provider provider, string localId, out AuthoredShipDeclaration? definition)
    {
    ...
    internal AuthoredShipDeclaration? Resolve(string owner, string localId)
    {
        _checkThread();
...
internal interface IAuthoredShipNative
{
    ...
    string? CreateShip(Guid session, string stationPoiId, AuthoredShipDeclaration declaration);
    ...
    void Maintain(Guid session, string stationPoiId, string unitId, AuthoredShipDeclaration declaration);
}

/// <summary>Keyed-owned reconciliation for moored authored ships; the API owns the persistent unit identity.</summary>
internal sealed class AuthoredShipCoordinator : IDisposable
{
    private readonly LifecycleHub _hub;
    private readonly AuthoredShipRegistry _definitions;
    private readonly IAuthoredShipNative _native;
    private readonly Func<Guid, bool> _persistenceReady;
    private readonly Action<string, string?> _protect;
    private readonly Action<Exception> _report;
    private readonly IDisposable _subscription;
    private readonly Dictionary<(string Owner, string Local, string Key), AuthoredShipOccurrence> _committed = new();
    ...
    internal AuthoredShipCoordinator(LifecycleHub hub, AuthoredShipRegistry definitions, IAuthoredShipNative native,
    ...
    internal void RestoreRows(Guid session, AuthoredShipOccurrence[] rows)
    {
        _hub.CheckThread();
    ...
    internal AuthoredShipOccurrence[] CaptureRows() { _hub.CheckThread(); return _committed.Values.ToArray(); }
    internal IReadOnlyList<AuthoredShipOccurrence> Occurrences(string owner)
    { _hub.CheckThread(); return _disposed ? Array.Empty<AuthoredShipOccurrence>() : _committed.Values.Where(o => o.Owner == owner).ToArray(); }
    internal AuthoredShipOccurrence? TryGetOccurrence(string owner, string localId, string occurrenceKey)
    ...
    internal (WorldStatus Status, AuthoredShipOccurrence? Row) Create(AuthoredShipRegistry.Provider provider,
        Guid expectedSession, string localId, string occurrenceKey, string stationPoiId)
    {
        ...
        try
        {
            ...
            var occurrence = new AuthoredShipOccurrence(provider.Owner, localId, occurrenceKey, declaration.Revision, stationPoiId, unitId);
    ...
    internal AuthoredShipState ReconstructionState(string owner, string localId, string occurrenceKey)
    {
        _hub.CheckThread();
        if (_disposed) return new AuthoredShipState(AuthoredSystemReconstructionStatus.Pending);
        if (_failed.Contains((owner, localId, occurrenceKey)))
            return new AuthoredShipState(AuthoredSystemReconstructionStatus.Failed, AuthoredSystemFailureReason.NativeMissing);
        return _committed.TryGetValue((owner, localId, occurrenceKey), out var row)
            ? Resolve(row) : new AuthoredShipState(AuthoredSystemReconstructionStatus.Pending);
    }
    ...
    private void TryProtect(AuthoredShipOccurrence occurrence)
    {
    ...
    internal AuthoredShipState Resolve(AuthoredShipOccurrence occurrence)
    {
        _hub.CheckThread();
        if (!_definitions.TryResolveMigration(occurrence.Owner, occurrence.LocalId, out var liveRevision, out var previousRevision))
            return new AuthoredShipState(AuthoredSystemReconstructionStatus.Failed, AuthoredSystemFailureReason.MissingDefinition);
        if (liveRevision != occurrence.Revision)
        {
            ...
            else return new AuthoredShipState(AuthoredSystemReconstructionStatus.Failed, AuthoredSystemFailureReason.RevisionMismatch);
        }
        if (!_persistenceReady(Session()))
            return new AuthoredShipState(AuthoredSystemReconstructionStatus.Failed, AuthoredSystemFailureReason.PersistenceUnavailable);
        try
        {
            return _native.ResolveShip(Session(), occurrence.StationPoiId, occurrence.UnitId)
                ? new AuthoredShipState(AuthoredSystemReconstructionStatus.Reconstructed, unitId: occurrence.UnitId)
                : new AuthoredShipState(AuthoredSystemReconstructionStatus.Pending);
        }
        catch (Exception error)
        { _report(error); return new AuthoredShipState(AuthoredSystemReconstructionStatus.Failed, AuthoredSystemFailureReason.NativeMissing); }
    }
    internal AuthoredSystemFailureReason PendingReason(Guid session)
