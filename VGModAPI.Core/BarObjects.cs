...
internal sealed partial class BarContentService
{
    ...
    private sealed class BarGame : IBars
    {
        ...
        internal void EnsureDeclarations()
        {
            ...
            foreach (var pair in _patrons.ToArray())
                if (!pair.Value.Owned.Live) _patrons.Remove(pair.Key);
        }
    }
    ...
    private sealed class Patron : IBarPatron
    {
        ...
        internal readonly DefinitionRegistration Owned;
        ...
        { _scope = scope; Owned = definition; _changed = new(scope.Owner._checkThread); }
        internal void Close() => _changed.Dispose();
        public IGame Game => _scope.Game;
        public IBarPatronDefinition Definition { get { _scope.Owner._checkThread(); return Owned; } }
        ...
        public BarPatronStatus Status
        {
            get
            {
                ...
                if (!Owned.Live || owner._storage == null || !owner.Availability.IsAvailable) return BarPatronStatus.Unavailable;
                if (!owner._persistence.Read(_scope.Session, out var rows)) return BarPatronStatus.Waiting;
                var state = rows.FirstOrDefault(row => row.Id == Owned.Id);
                if (state == null) owner._transient.TryGetValue(Owned.Id, out state);
        ...
        public BarResult Remove() => Schedule(() => Owned.Lease.Remove(_scope.Session, Owned.Id.LocalId));
        public BarResult Restore() => Schedule(() =>
        {
            ...
            var refusal = owner.Guard(Owned.Lease, _scope.Session);
            if (refusal != null) return refusal;
            owner._persistence.Read(_scope.Session, out var rows);
            var existing = rows.FirstOrDefault(row => row.Id == Owned.Id);
            if (existing != null)
            {
                if (Owned.Lease.Definitions[Owned.Id.LocalId].Retention != BarPatronRetention.Persistent)
                ...
                var restored = owner._persistence.Put(_scope.Session, Owned.Id.Provider, existing.WithRemoved(false));
                if (restored) owner.Changed();
                return new BarResult(restored ? BarStatus.Succeeded : BarStatus.Unavailable);
            }
            return Owned.Lease.Place(_scope.Session, Owned.Id.LocalId);
        });
        private BarResult Schedule(Func<BarResult> action)
        {
            var owner = _scope.Owner; owner._checkThread();
            if (!Game.IsActive) return _last = new(BarStatus.GameEnded);
            if (!Owned.Live || owner._storage == null) return _last = new(BarStatus.Unavailable);
            var result = new BarResult(BarStatus.Queued)
            { Ended = () => !Game.IsActive ? BarStatus.GameEnded : !Owned.Live ? BarStatus.Unavailable : null };
            Pending = true; _last = result;
            owner._hub.Gameplay.Enqueue(_scope.Session, Owned.Id.Provider, () =>
            {
                try
                {
                ...
                { result.Finish(BarStatus.Unavailable, "The bar action failed."); owner._hub.Gameplay.Report(Owned.Id.Provider, error); }
                ...
                _changed.Publish(owner._hub, _scope.Session, Owned.Id.Provider, this, () => Game.IsActive && Owned.Live, owner._storage.Registration, Owned.Lease.SaveData);
            }, () => Game.IsActive && Owned.Live, owner._storage.Registration, Owned.Lease.SaveData);
            return result;
        }
    }
}
