...
internal sealed class BoardingCommandService : IBoardingCommands, IDisposable
{
    ...
    internal BoardingCommandService(LifecycleHub hub, IBoardingEvents events, IBoardingCommandBackend backend, Func<bool> rulesEvaluating)
    ...
    {
        _hub.CheckThread();
        if (_disposed) return Result(BoardingCommandStatus.IntegrationUnavailable);
    ...
    private bool Current(Controller controller) => !controller.Disposed && !_disposed &&
