...
internal static class StoryDefinitionCodec
{
    ...
    internal static byte[] Encode(StoryMissionDefinition definition)
    {
        ...
        foreach (var step in definition.Steps)
        {
            ...
            foreach (var objective in step.Objectives)
            {
                ...
                Text(writer, objective.Description);
            }
        }
        writer.Write((byte)definition.Rewards.Count);
        foreach (var reward in definition.Rewards) { writer.Write((byte)reward.Kind); writer.Write(reward.Amount); }
    ...
    internal static StoryMissionDefinition Decode(byte[] payload)
    {
        ...
        if (reader.ReadByte() != 1) throw new InvalidDataException("Unknown definition format.");
        ...
        for (int index = 0; index < steps.Length; index++)
        {
            ...
            for (int item = 0; item < objectives.Length; item++)
            {
                ...
                {
                    StoryObjectiveKind.TravelToPoi when amount == 0 && text == null => StoryObjective.TravelTo(target!, visit),
                    StoryObjectiveKind.CollectCredits when target == null && visit == 0 && text == null => StoryObjective.CollectCredits(amount),
                    StoryObjectiveKind.KillEnemies when target == null && visit == 0 && text == null => StoryObjective.KillEnemies(amount),
                    StoryObjectiveKind.Scripted when target == null && visit == 0 => StoryObjective.Scripted(key!, text!, amount),
        ...
        for (int index = 0; index < rewards.Length; index++) rewards[index] = new StoryReward((StoryRewardKind)reader.ReadByte(), reader.ReadInt32());
