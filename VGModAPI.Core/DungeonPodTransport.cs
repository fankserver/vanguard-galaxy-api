...
internal sealed class DungeonPodTransport
{
    ...
    internal string DonorShipId { get; }
    ...
    internal DungeonPodTransport(string nativePodId, bool pendingReinforcement, IReadOnlyDictionary<string, int> outboundCrew, IEnumerable<float> pose, string donorShipId = "")
    {
        ...
        if (donorShipId == null || donorShipId.Length > 128 || donorShipId.IndexOf('\0') >= 0) throw new ArgumentException("Invalid donor ship identity.");
        DonorShipId = donorShipId;
