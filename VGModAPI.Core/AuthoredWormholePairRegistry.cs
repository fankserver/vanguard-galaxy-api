...
internal sealed class AuthoredWormholePairRegistry : IDisposable
{
    internal sealed class Provider : IDisposable
    {
        private readonly AuthoredWormholePairRegistry _registry;
        internal string Owner { get; }
        internal Provider(AuthoredWormholePairRegistry registry, string owner) { _registry = registry; Owner = owner; }
        internal bool Register(AuthoredWormholePairDefinition definition, AuthoredWormholePairDefinition? previous = null)
        {
    ...
    private readonly Dictionary<(string Owner, string Local), AuthoredWormholePairDeclaration> _definitions = new();
    private readonly Dictionary<(string Owner, string Local), AuthoredWormholePairDeclaration> _previous = new();
    private long _revision;
    private bool _disposed;
    internal long Revision { get { _checkThread(); return _revision; } }
    internal AuthoredWormholePairRegistry(StoryHostAuthenticator authenticate, Action checkThread)
    ...
    internal bool Register(Provider provider, AuthoredWormholePairDefinition definition, AuthoredWormholePairDefinition? previous = null)
        => Register(provider, Map(definition ?? throw new ArgumentNullException(nameof(definition))), previous == null ? null : Map(previous));
    private static AuthoredWormholePairDeclaration Map(AuthoredWormholePairDefinition definition)
        => new(definition.LocalId, definition.Revision, definition.Name);
    private bool Register(Provider provider, AuthoredWormholePairDeclaration definition, AuthoredWormholePairDeclaration? previous)
    {
        _checkThread();
    ...
    internal bool TryResolve(Provider provider, string localId, out AuthoredWormholePairDeclaration? definition)
    {
        _checkThread();
        definition = null;
