...
internal static class ModUpdateWireChecks
{
    ...
    {
        private readonly IModFeedTransport _inner;
        internal Observed(IModFeedTransport? inner) { _inner = inner ?? new HttpModFeedTransport(); }
    ...
    internal static async Task RunAsync(string revision, Action<string> record, CancellationToken cancellation, IModFeedTransport? supplied = null)
    {
        var sink = record;
        ...
        using var transport = new Observed(supplied);
        var client = new ModFeedClient(transport);
        foreach (var channel in new[] { "stable", "experimental" })
        {
            var feed = await client.FetchAsync(root + channel + ".json", ModUpdateChecks.Id, channel, cancellation);
        ...
        await client.FetchAsync("https://github.com/fankserver/vanguard-galaxy-api/raw/" + revision + "/tools/fixtures/mod-updates/stable.json",
            ModUpdateChecks.Id, "stable", cancellation);
        ...
        {
            var rejected = false;
            try { await client.FetchAsync(root + file, ModUpdateChecks.Id, "stable", cancellation); }
            catch (FormatException error) { rejected = file != "oversized.json" || error.Message == "Update feed is oversized."; }
