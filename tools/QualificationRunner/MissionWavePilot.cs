    ...
    private IEnumerable<object?> CheckGuildWaves()
    {
        ...
        foreach (var force in new[] { false, true })
        {
            ...
            try
            {
                ...
                foreach (var frame in Wait(() => ReferenceEquals(SpGet(AccessTools.TypeByName("Source.Galaxy.MapPointOfInterest"), "current"), poi), "wave arrival " + kind)) yield return frame;
                foreach (var frame in Settle()) yield return frame;
