...
try {
    ...
    $record = @{ schema='world-empty-run-v1'; root=$root; gameDirectory=$env:TEMP; runId=$run.ToString('D'); phase='create'; reviewedHead=$head; authorizationSha256=('b' * 64); gameInventory=@{ 'input.dll'=('F:' + ('c' * 64)) }; saveInventory=@{ 'fixture-a.save'=('F:' + ('d' * 64)) }; stateInventory=@{} }
