...
function Assert-BlueprintPinSelection([string]$Root, $Provenance) {
    ...
    if (!$selected) { return }
    if (!$Provenance.forgeReadProbe -or $Provenance.forgeUiProbe -or $Provenance.forgeCommandProbe -or $Provenance.refineryProbe -or $Provenance.forgeDeliveryProbe -or $Provenance.forgePersistenceProbe -or [IO.File]::ReadAllText($marker) -cne 'blueprint-pin-v9') { throw 'Invalid Blueprint Pin selection.' }
...
function Assert-BlueprintPinReceipt([string]$Root, $Provenance) {
    ...
    if ($lines.Count -ne 3 -or $lines[0] -cne 'PASS' -or $lines[1] -cne 'blueprint-pin-v9' -or $lines[2] -cne 'pin-batch-exact-variant-navigation-close-producer-routes-queue-partial-cancel-multiunit-reload-saveas-switch-inspector-isolation-context-generated-routing') { throw 'Incomplete Blueprint Pin receipt.' }
    $routing = Join-Path $Root 'forge-routing.txt'
    if (!(Test-Path -LiteralPath $routing -PathType Leaf) -or (Get-Item -LiteralPath $routing).Length -gt 256 -or ((Get-Item -LiteralPath $routing).Attributes -band [IO.FileAttributes]::ReparsePoint)) { throw 'Missing or invalid Forge routing receipt.' }
    $routingLines = @(Get-Content -LiteralPath $routing)
    if ($routingLines.Count -ne 3 -or $routingLines[0] -cne 'PASS' -or $routingLines[1] -cne 'armory-and-cargo-delivered' -or $routingLines[2] -cne 'preference-restored') { throw 'Incomplete Forge routing receipt.' }
    $generated = Join-Path $Root 'forge-generated.txt'
    if (!(Test-Path -LiteralPath $generated -PathType Leaf) -or (Get-Item -LiteralPath $generated).Length -gt 256 -or ((Get-Item -LiteralPath $generated).Attributes -band [IO.FileAttributes]::ReparsePoint)) { throw 'Missing or invalid generated Forge receipt.' }
    $generatedLines = @(Get-Content -LiteralPath $generated)
    if ($generatedLines.Count -ne 3 -or $generatedLines[0] -cne 'PASS' -or $generatedLines[1] -cne 'generated-equipment-delivered' -or $generatedLines[2] -cne 'level-and-inventory-reconciled') { throw 'Incomplete generated Forge receipt.' }
    ...
    if ($isolationLines.Count -ne 3 -or $isolationLines[0] -cne 'PASS' -or $isolationLines[1] -cne 'throwing-listener-isolated' -or $isolationLines[2] -cne 'following-listener-delivered') { throw 'Incomplete Forge callback receipt.' }
