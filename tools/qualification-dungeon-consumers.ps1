...
function Install-DungeonConsumers([string]$Manifest, [string]$Root, [string]$Plugins, [string]$Cecil) {
    ...
    if ($m.PSObject.Properties['mode'] -and $m.mode -ceq 'SaveLoad') { [IO.File]::WriteAllText((Join-Path $Root 'dungeon-save.enabled'), 'dungeon-save-v4') }
...
function Assert-DungeonConsumerSelection([string]$Root, $Provenance) {
    ...
    if ((Test-Path -LiteralPath $saveMarker) -ne [bool]$save -or ($save -and [IO.File]::ReadAllText($saveMarker) -cne 'dungeon-save-v4')) { throw 'Dungeon save selection mismatch.' }
...
function Assert-DungeonConsumerReceipt([string]$Root, $Provenance) {
    ...
    if (Test-Path -LiteralPath (Join-Path $Root 'dungeon-save.enabled')) {
        ...
        if (($save -join "`n") -cne "PASS`ndungeon-save-v4`nsave-failures-transitions-rollback-provider-recovery") { throw 'Incomplete dungeon save receipt.' }
        ...
        $provider = [IO.File]::ReadAllLines((Join-Path $Root 'dungeon-provider.txt'))
        if (($provider -join "`n") -cne "PASS`ndungeon-provider-v1`nabsent-registration-version-refusal-matching-behavior-once") { throw 'Incomplete dungeon provider recovery receipt.' }
    }
