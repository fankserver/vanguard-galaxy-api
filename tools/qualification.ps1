...
if ($Action -eq 'Prepare') {
    ...
    if ($ForgePersistenceProbe) { $ForgeCommandProbe = $true }
    if ($ForgeCommandProbe) { $ForgeReadProbe = $true }
    if ($ForgeReadProbe) {
        $otherSwitches = @($PSBoundParameters.Keys | Where-Object { $PSBoundParameters[$_] -is [Management.Automation.SwitchParameter] -and $PSBoundParameters[$_].IsPresent -and $_ -notin @('ForgeReadProbe','ForgeCommandProbe','ForgePersistenceProbe','Diagnostics') })
